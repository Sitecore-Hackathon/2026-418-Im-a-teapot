...
const urlPrefix = "https://coordinatorapi-bzacc3gkdgh7ehdd.westeurope-01.azurewebsites.net/api/receive/hook/";
...
const createUrl = (instanceId: string) => urlPrefix + instanceId;
...
export const makeUpdateWebhookQuery = (sitecoreContextId: string, instanceId: string, existingItemId: string) => ({
    ...
    body: {
        query: updateWebhookQuery.replace('\$url', createUrl(instanceId)),
...
export const makeCreateWebhookQuery = (sitecoreContextId: string, instanceId: string) => ({
    query: { sitecoreContextId },
    body: {
        query: createWebhookQuery.replace('\$url', createUrl(instanceId)),
